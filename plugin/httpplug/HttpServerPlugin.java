...
public class HttpServerPlugin extends Plugin
{
				...
				if (player.getLocalLocation().distanceTo(npc.getLocalLocation()) <= MAX_DISTANCE)
				{
					...
					if (localXY[0] > 1 && localXY[0] < MAX_DISTANCE)
					{
						if (localXY[1] > 1 && localXY[1] < MAX_DISTANCE)
						{
							...
							JsonObject npcCanvasLocation = new JsonObject();
							JsonObject npcWorldLocation = new JsonObject();
							npcCanvasLocation.addProperty("x", localXY[0]);
							npcCanvasLocation.addProperty("y", localXY[1]);
							npcWorldLocation.addProperty("x", npc.getWorldLocation().getX());
							npcWorldLocation.addProperty("y", npc.getWorldLocation().getY());


							JsonObject npcJson = new JsonObject();
							npcJson.addProperty("name", name);
							npcJson.add("canvas", npcCanvasLocation);
							npcJson.add("coordinates", npcWorldLocation);

							object.add(name + "_" + i, npcJson);
						}
		...
		if (npc != null)
	{
		...
		int playerPlane = player.getWorldLocation().getPlane();
		Point playerCanvasFinal = localToCanvas(client, player.getLocalLocation(), playerPlane);
		JsonObject object = new JsonObject();
		JsonObject camera = new JsonObject();
		JsonObject worldPoint = new JsonObject();
		JsonObject mouse = new JsonObject();
		JsonObject health_stats = new JsonObject();
		...
		health_stats.addProperty("current health", client.getBoostedSkillLevel(Skill.HITPOINTS));
		health_stats.addProperty("max health", client.getRealSkillLevel(Skill.HITPOINTS));
		...
		worldPoint.addProperty("canvas_x", playerCanvasFinal.getX());
		worldPoint.addProperty("canvas_y", playerCanvasFinal.getY());
		...
		object.add("health", health_stats);
		exchange.sendResponseHeaders(200, 0);
		try (OutputStreamWriter out = new OutputStreamWriter(exchange.getResponseBody()))
		{
			RuneLiteAPI.GSON.toJson(object, out);
			...
			for (GameObject gameObject : gameObjects)
			{
				if (gameObject != null && gameObject.getSceneMinLocation().equals(tile.getSceneLocation()))
				{
					if (player.getLocalLocation().distanceTo(gameObject.getLocalLocation()) <= MAX_DISTANCE)
					{
						...
						JsonObject objectCoordinates = new JsonObject();
						JsonObject objectCanvas = new JsonObject();
						objectCoordinates.addProperty("x", gameObject.getWorldLocation().getX());
						objectCoordinates.addProperty("y", gameObject.getWorldLocation().getY());
						objectCanvas.addProperty("x", objCanvasFinal.getX());
						objectCanvas.addProperty("y", objCanvasFinal.getY());

						JsonObject gameObjectJson = new JsonObject();
						gameObjectJson.addProperty("id", name);
						gameObjectJson.add("coordinates", objectCoordinates);
						gameObjectJson.add("canvas", objectCanvas);
						obj_name.add(gameObjectJson.toString());
					}
				}
				i = i +1;
			}
