...
import random
import time
from sly_scripts import functions
from sly_scripts.functions import Image_count
from sly_scripts.functions import image_Rec_clicker
from sly_scripts.functions import release_drop_item
from sly_scripts.functions import drop_item
from sly_scripts.functions import Image_to_Text
from sly_scripts.functions import invent_crop
from sly_scripts.functions import resizeImage
from sly_scripts.functions import skill_lvl_up
from sly_scripts.functions import spaces
from sly_scripts.functions import random_combat
from sly_scripts.functions import random_quests
from sly_scripts.functions import random_skills
from sly_scripts.functions import random_inventory
from sly_scripts.functions import random_breaks
...
def timer_countdown():
    ...
    for i in range(final):
        ...
        print(bcolors.OK + f'\r[%-10s] %d%%' % ('=' * round((i / final) * 10), round((i / final) * 100)),
              f'time left: {(t_end - time.time()) / 60 :.2f} mins | coords: {coords} | status: {fishing_text} | fish: {fish_count} | clues: {clue_count} | {actions}',
              end='')
        time.sleep(1)
...
def powerfisher(fish_type, Run_Duration_hours=6):
    ...
    while time.time() < t_end:
        ...
        if fish_type == 'prawn_fish':
            fish_count = functions.invent_count(fish_type + '.png', 0.95) + functions.invent_count('anch_fish.png',
                                                                                                   0.95)
