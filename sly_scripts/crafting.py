...
from sly_scripts.functions import pick_item
...
from sly_scripts.functions import find_Object_precise
from sly_scripts.functions import exit_bank
from sly_scripts.functions import deposit_secondItem
...
def timer_countdown():
    ...
    for i in range(invent_count):
        ...
        print(bcolors.OK + f'\r[%-10s] %d%%' % ('=' * round((i / invent_count) * 10), round((i / invent_count) * 100)),
              f'bars left: {inv} | coords: {coords} | status: {actions}', end='')
        time.sleep(1)
